...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        # i_t
        self.U_i_1 = nn.Linear(emb_size, n_hidden, bias=False)
        self.V_i_1 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_i_1 = nn.Parameter(torch.Tensor(n_hidden))

        self.U_i_2 = nn.Linear(emb_size, n_hidden, bias=False)
        self.V_i_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_i_2 = nn.Parameter(torch.Tensor(n_hidden))

        # f_t
        self.U_f_1 = nn.Linear(emb_size, n_hidden, bias=False)
        self.V_f_1 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_f_1 = nn.Parameter(torch.Tensor(n_hidden))

        self.U_f_2 = nn.Linear(emb_size, n_hidden, bias=False)
        self.V_f_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_f_2 = nn.Parameter(torch.Tensor(n_hidden))

        # c_t
        self.U_c_1 = nn.Linear(emb_size, n_hidden, bias=False)
        self.V_c_1 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_c_1 = nn.Parameter(torch.Tensor(n_hidden))

        self.U_c_2 = nn.Linear(emb_size, n_hidden, bias=False)
        self.V_c_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_c_2 = nn.Parameter(torch.Tensor(n_hidden))

        # o_t
        self.U_o_1 = nn.Linear(emb_size, n_hidden, bias=False)
        self.V_o_1 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_o_1 = nn.Parameter(torch.Tensor(n_hidden))

        self.U_o_2 = nn.Linear(emb_size, n_hidden, bias=False)
        self.V_o_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_o_2 = nn.Parameter(torch.Tensor(n_hidden))

        self.U_h = nn.Linear(n_hidden, n_hidden, bias=False)
        self.V_h = nn.Linear(n_hidden, n_hidden, bias=False)
    ...
    def forward(self, X, h_t, c_t):
        ...
        C=c_t
        for x_t in X:
            i_t = torch.sigmoid(self.U_i_1(x_t) + self.V_i_1(h_t) + self.b_i_1)
            f_t = torch.sigmoid(self.U_f_1(x_t) + self.V_f_1(h_t) + self.b_f_1)
            g_t = torch.tanh(self.U_c_1(x_t) + self.V_c_1(h_t) + self.b_c_1)
            o_t = torch.sigmoid(self.U_o_1(x_t) + self.V_o_1(h_t) + self.b_o_1)
            c_t = f_t * c_t + i_t * g_t
            h_t = o_t * torch.tanh(c_t)
            H = torch.sigmoid(self.U_h(h_t) + self.V_h(H) + self.b_h)
            I = torch.sigmoid(self.U_i_2(x_t) + self.V_i_2(H) + self.b_i_2)
            F = torch.sigmoid(self.U_f_2(x_t) + self.V_f_2(H) + self.b_f_2)
            G = torch.tanh(self.U_c_2(x_t) + self.U_c_2(H) + self.b_c_2)
            O = torch.sigmoid(self.U_o_2(x_t) + self.V_f_2(H) + self.b_o_2)
            C = F * C + I * G
            H = O * torch.tanh(C)
...
def train_LSTMlm():
    ...
    for epoch in range(all_epoch):
        ...
        for input_batch, target_batch in zip(all_input_batch, all_target_batch):
            optimizer.zero_grad()
            # hidden_state 为短期记忆状态 [batch_size, n_hidden]
            h_t = torch.zeros(batch_size, n_hidden,device=device)
            # cell_state 为长期记忆状态 [batch_size, n_hidden]
